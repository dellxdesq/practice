...
class OllamaModel:
    ...
    def generate_answer_from_segment(self, segment, query_text):
        # Формируем сообщение для модели, добавляя контекст и инструкцию для ответа только на основе контекста
        message = f"""
        Ты должен ответить на вопрос, используя только следующий контекст. Если ты не знаешь ответа на основе контекста, скажи "Извините, я не знаю ответа на этот вопрос. Отвечать надо только на русском языке."
