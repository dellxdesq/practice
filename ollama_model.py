...
class OllamaModel:
    ...
    def rank_segments(self, segments, query_text):
        ...
        Если ты не нашел нужный сегмент, то есть вообще нет сегментов которые могли бы содержать ответ на вопрос, ты должен ответить "Извините я не знаю ответа на вопрос"
    ...
    def summarize_segment(self, segment_text):
        ...
        также предложение не должно начинаться со слов содержание или краткое содержание, в твоем ответе это просто описание того, что написано в сегменте.
